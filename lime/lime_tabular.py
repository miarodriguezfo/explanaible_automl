...
class LimeTabularExplainer_MR(LimeTabularExplainer):
    ...
    def explain_instance(self,
        ...
        domain_mapper = TableDomainMapper(feature_names,
                                          ...
                                          discretized_feature_names=discretized_feature_names,
                                          feature_indexes=feature_indexes)
        ret_exp = explanation.Explanation_MR(domain_mapper,
                                          mode=self.mode,
                                          class_names=self.class_names)
        if self.mode == "classification":
            ret_exp.neighborhood_data = inverse
            ret_exp.neighborhood_scaled_data=scaled_data
            ret_exp.predict_proba = yss[0]
            if top_labels:
                labels = np.argsort(yss[0])[-top_labels:]
                ret_exp.top_labels = list(labels)
        ...
        for label in labels:
            (ret_exp.intercept[label],
             ret_exp.local_exp[label],
             ret_exp.score, ret_exp.local_pred, ret_exp.model) = self.base.explain_instance_with_data(
                    scaled_data,
                    yss,
                    distances,
                    label,
    ...
    def global_explanation(self,
        ...
        score_list = [None] * data_len
        for i in range(0, data_len):
            # if i%100 ==0:
            #     print(i)
            exp = self.explain_instance(data.values[i], predict_fn,num_samples=num_samples, num_features=num_features,
             top_labels=top_labels, labels=labels, distance_metric=distance_metric, model_regressor=model_regressor)
            lime_model_predictios=exp.model.predict(exp.neighborhood_scaled_data)
            best_rating=np.argsort(predict_fn(exp.neighborhood_data[0].reshape(1, -1)))[-1][-1:]
            tpot_model_predictions=predict_fn(exp.neighborhood_data)[:,best_rating].T[0] 
            score_list[i]=prediction_score(lime_model_predictios,tpot_model_predictions)
        total_error =  sum(score_list) 
